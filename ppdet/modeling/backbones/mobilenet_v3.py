...
class MobileNetV3(nn.Layer):
    ...
    def __init__(
            ...
            scale=1.0,
            model_name="large",
            feature_maps=[6, 12, 15],   # 默认是YOLOv3的输出
        ...
        if model_name == "large":
            self.cfg = [
                ...
                [5, 120, 40, True, "relu", 1],  # YOLOv3 output 6
                [3, 240, 80, False, "hard_swish", 2],  # RCNN output 
                [3, 200, 80, False, "hard_swish", 1],
                [3, 184, 80, False, "hard_swish", 1],
                [3, 184, 80, False, "hard_swish", 1],
                [3, 480, 112, True, "hard_swish", 1],
                [3, 672, 112, True, "hard_swish", 1],  # YOLOv3 output 12
                [5, 672, 160, True, "hard_swish", 2],  # SSD/SSDLite/RCNN output
                [5, 960, 160, True, "hard_swish", 1],
                [5, 960, 160, True, "hard_swish", 1],  # YOLOv3 output 15
            ]
        ...
        self._out_channels = []
        self._out_strides = [8, 16, 32]  # 这个是根据resnet的stride
        self.block_list = []
        i = 0
    ...
    def forward(self, inputs, flag=0):
        if flag == 0:   # 通过flag来区分输入的图片是IR还是VIS, flag为0代表处理正常网络
            x = self.conv1(inputs['image'])
        elif flag == 1:
            x = self.conv1(inputs['vis_image'])
        elif flag == 2:
            x = self.conv1(inputs['ir_image'])
        outs = []
    ...
    def out_shape(self):
        ...
        # print(self._out_channels)   # [40, 112, 160]     
        return [
            ShapeSpec(
                channels=self._out_channels[i], stride=self._out_strides[i]) for i in range(len(self._out_channels))
        ]   # 主要修改这里，用列表生成式，模仿Resnet的out_shape
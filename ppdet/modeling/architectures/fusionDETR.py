...
class fusionDETR(BaseArch):
    ...
    def _forward(self):
        ...
        ir_img = 2, 3, 736, 736   
        """
        vis_body_feats = self.backbone_vis(self.inputs,1)   # vis_body_feats = [2, 512, 92, 92], [2, 1024, 46, 46], [2, 2048, 23, 23]
        ...
        visir_body_feats = [v + i for v, i in zip(vis_body_feats, ir_body_feats)]   ### 最简单的想法，把编码器的输出相加 列表元素相加要用列表生成式
        out_transformer = self.transformer(visir_body_feats, pad_mask, self.inputs)
